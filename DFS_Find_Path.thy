...
locale fp0 = param_DFS G "fp0_params P"
  ...
  fixes T
  assumes \<open>G = G\<close> \<open>T = T\<close>
begin
...
lemma fp0I: assumes "fb_graph G" "T = T" shows "fp0 G T"
proof - interpret fb_graph G by fact show ?thesis by unfold_locales auto qed

locale fp0_invar = fp0 + 
  DFS_invar where param = "fp0_params P"

lemma fp0_invar_eq[simp]: 
  "DFS_invar G (fp0_params P) s \<longleftrightarrow> fp0_invar G P T s"
proof (intro ext iffI)
  fix s
  assume "DFS_invar G (fp0_params P) s"
  interpret DFS_invar G "fp0_params P" s by fact
  show "fp0_invar G P T s" by unfold_locales auto
next
  fix s
  assume "fp0_invar G P T s"
  interpret fp0_invar G P T s by fact
...
context fp0 begin
  lemma fp0_correct: "it_dfs \<le> SPEC (\<lambda>s. case ppath s of 
    ...
    apply clarsimp
    using fp0_invar.fp0_correct fp0_invar_eq by fastforce
end
...
definition find_path0_spec
  :: "('v, _) graph_rec_scheme \<Rightarrow> ('v \<Rightarrow> bool) \<Rightarrow> _ \<Rightarrow> 'v fp_result nres"
  \<comment> \<open>Searches a path from the root nodes to some target node that satisfies a 
      ...
      are returned\<close>
where
  "find_path0_spec G P T \<equiv> do {
    ASSERT (fb_graph G);
    SPEC (find_path0_pred G P)
  }"

definition find_path0 
  :: "('v, 'more) graph_rec_scheme \<Rightarrow> ('v \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> 'v fp_result nres"
  where "find_path0 G P T \<equiv> do {
  ASSERT (fp0 G T);
...
lemma find_path0_correct:
  shows "find_path0 G P T \<le> find_path0_spec G P T"
...
proof (rule le_ASSERT_defI1[OF find_path0_restr_spec_def], clarify)
  ...
  interpret fp0 "graph_restrict G R" by unfold_locales simp
