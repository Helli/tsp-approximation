...
begin
  ...
  abbreviation "ind E' \<equiv> \<lparr>nodes=V, edges=E'\<rparr>"
  ...
  lemma ind_valid_graph: "\<And>E'. E' \<subseteq> E \<Longrightarrow> valid_graph (ind E')"
    by (simp add: subgraph_def valid_subgraph)
  ...
  lemma add_edge_ind: "(a,w,b)\<in>E \<Longrightarrow> add_edge a w b (ind F) = ind (insert (a,w,b) F)"
    by (simp add: E_validD insert_absorb)
  ...
  sublocale s: Kruskal_interface E V
  ...
  next
    ...
    from 7 have uv: "u\<in>V" "v\<in>V" using E_valid by blast+
  ...
  next
    ...
    from 10(4) have ad_eq: "add_edge a w b (ind F) = ind (insert e F)"
      using e unfolding add_edge_def by (simp add: E_validD insert_absorb)
  ...
  next
    ...
    with 14 have "a\<in>V" and p: "(a,w,a): edges (ind (insert e F))"
      by (auto simp: E_validD(2))
  ...
  next
    ...
    then show ?case apply auto
        subgoal for w apply(rule exI[where x="[(a, w, b)]"])
          using finite_weighted_graph.ind_valid_graph finite_weighted_graph_axioms valid_graph.is_path_undir_simps(2) by fastforce
        subgoal for w apply(rule exI[where x="[(a, w, b)]"]) apply simp
          using E_validD(1) by blast
        done
  qed
...
sublocale finite_weighted_graph "\<lparr>nodes=fst ` set L \<union> (snd \<circ> snd) ` set L, edges=set L\<rparr>"
  apply standard by simp_all blast

lemma max_node_is_Max_V: " E =  set la \<Longrightarrow> max_node la = Max (insert 0 V)"
  by (auto simp: max_node_def)
